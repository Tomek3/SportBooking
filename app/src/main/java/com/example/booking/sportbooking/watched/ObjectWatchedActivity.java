...
public class ObjectWatchedActivity extends BaseActivity {

    private int selectedPosition = -1;
    private Reservation selectedReservation = null;
    private ListView listView = null;
    private ObjectWatchedAdapter adapter = null;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if(R.id.action_delete == item.getItemId())
        {
            ...
            deleteWatched(getSharedPreferences("UserInfo", 0).getInt("UserId",-1), selectedReservation.getId());
    ...
    public void loadAdapterData(){
        ...
        Call<List<Reservation>> call = apiService.doGetListUserObjectWatched(userId.toString());
        call.enqueue(new Callback<List<Reservation>>() {
            @Override
            public void onResponse(Call<List<Reservation>>call, Response<List<Reservation>> response) {
                List<Reservation> reservations = response.body();

                adapter = new ObjectWatchedAdapter(getBaseContext(), reservations);
    ...
    public void deleteWatched(Integer userId, Integer watchedId){
        ...
        params.put("resId", watchedId.toString());

        client.get(ReservationService.getAbsoluteUrl("reservation/watched/delete"),params ,new TextHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, String response) {
                ...
                try {
                    ...
                    if(obj.getBoolean("status")){
                        Toast.makeText(getBaseContext(), R.string.watchedDeleted, Toast.LENGTH_LONG).show();
                        removeItemFromAdapter();
                    }
                    else{
                        Toast.makeText(getBaseContext(), R.string.watchedDeletedError, Toast.LENGTH_LONG).show();
                    }
